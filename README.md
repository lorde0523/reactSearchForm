...
특수 값은 `serialize`, `deserialize`, `formatDisplay`를 전달하면 저장, 복원, 팝업 표시에서도 같은 선언을 사용합니다. 단일 `CheckboxField`는 체크 여부를 `true`/`false`로 모두 저장하고 미체크값은 팝업에서 기본으로 숨깁니다. `CheckboxGroupField`는 선택된 option label만 `/`로 연결합니다.
